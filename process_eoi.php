...
/* II. Data Handling */
  ...
  $otherskills_input = sanitizeInput($_POST['otherskills_input']);  // other skills input
...
/* III. Validation */
  ...
  // 3. Last Name
    if (empty($lastName)) {
      ...
      $errors['lastName'] = 'Last name must be no more than 20 alphabetic characters';
    }
  ...
  // 12. Other Skills
    if (isset($_POST['otherskills_checkbox']) && empty($otherskills_input)) {
      $errors['otherskills_input'] = 'Please enter your other skills'; 
    }

  // 13. Error Handling (if exists)
    if (!empty($errors)) {
      // Stored submitted data in session
...
/* IV. Database Operations */
  ...
  $query_template = "INSERT INTO eoi (JobReference, firstName, lastName, dob, gender, street, suburb, state, postcode, email, phone, skills, otherSkills, Status) 
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'New')";
  ...
  if ($stmt) {
    ...
    mysqli_stmt_bind_param(
      $stmt, 
      'sssssssssssss', 
      $jobRef, $firstName, $lastName, $dob, $gender, $street, $suburb, $state, $postcode, $email, $phone, $skillsString, $otherSkills
    );
    ...
    if ($executeResult) {
      ...
      include "menu.inc";
      ?>
<div class='success_submit_container'>
  <div class='success-message'>
    <p>Thank you for your application.</p>
    <div class='eoi-number'>
      <strong>Your unique EOI Number is:</strong>
      <span class='number'><?php echo $newEOInumber; ?></span>
    </div>
